...
contract FlightSuretyApp
{
    ...
    public
    {
        ...
        flightSuretyData.registerAirline(firstAirline, msg.sender);

        flightSuretyData.registerFlight(firstAirline, "Flight 01", 1657534498);
        require(
            flightSuretyData.isFlightRegistered(firstAirline, "Flight 01", 1657534498),
            "Flight could not be registered"
        );

        flightSuretyData.registerFlight(firstAirline, "Flight 02", 1657534509);
        require(
            flightSuretyData.isFlightRegistered(firstAirline, "Flight 02", 1657534509),
            "Flight could not be registered"
        );

        flightSuretyData.registerFlight(firstAirline, "Flight 03", 1657534516);
        require(
            flightSuretyData.isFlightRegistered(firstAirline, "Flight 03", 1657534516),
            "Flight could not be registered"
        );
    }
    ...
    function voteForAirline(address airline) external {
        // pass origin sender, so it does not change to FlightSuretyApp
        flightSuretyData.voteForAirline(airline, msg.sender);
    }


    function payFundForAirline() external payable {
        // pass origin sender, so it does not change to FlightSuretyApp
        flightSuretyData.payFundForAirline.value(msg.value)(msg.sender);
    }
...
{
    function registerAirline(address newAirlineAddress, address currentAirlineAddress) external;
    function registerFlight(address airline, string flightId, uint256 timestamp) external;
    function isFlightRegistered(address airline, string flightId, uint256 timestamp) external view returns (bool);
    ...
    function withdrawInsurance(address airline, string flight, uint256 timestamp, address passenger) external payable;
    function repayPassengerForFlight(address airline, string flight, uint256 timestamp) external payable;
